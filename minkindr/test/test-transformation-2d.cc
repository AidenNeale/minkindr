...
TEST(TestTransformation2D, TestInitialization) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR(T.getPosition(), Eigen::Vector2d(0.0, 0.0), kEpsilon));
  ...
  const Eigen::Matrix<double, 3, 3> transformation_matrix =
      (Eigen::Matrix<double, 3, 3>() << std::cos(r.angle()), -std::sin(r.angle()), t.x(),
       std::sin(r.angle()), std::cos(r.angle()), t.y(), 0.0, 0.0, 1.0)
          .finished();
  const Transformation2D T_from_mat(transformation_matrix);
  EXPECT_TRUE(
      EIGEN_MATRIX_NEAR(transformation_matrix, T_from_mat.getTransformationMatrix(), kEpsilon));
  EXPECT_TRUE(
      EIGEN_MATRIX_NEAR(transformation_matrix, T_rot_pos.getTransformationMatrix(), kEpsilon));

  const Eigen::Matrix<double, 3, 3> invalid_transformation_matrix =
      (Eigen::Matrix<double, 3, 3>() << 1.0, 1.0, t.x(), 1.0, 1.0, t.y(), 0.0, 0.0, 1.1).finished();
  EXPECT_DEATH(const Transformation2D T_invalid(invalid_transformation_matrix), "");
}
...
TEST(TestTransformation2D, TestGettersAndSetters) {
  ...
  EXPECT_TRUE(
      EIGEN_MATRIX_NEAR(T.asVector(), (Eigen::Vector3d() << r.angle(), t).finished(), kEpsilon));
...
Eigen::Vector2d fromHomogeneous(const Eigen::Vector3d& v) { return v.head<2>() / v[2]; }
...
TEST(TestTransformation2D, TestComposition) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR(T * T * v, fromHomogeneous(T_mat * T_mat * vh), kEpsilon));

  EXPECT_TRUE(
      EIGEN_MATRIX_NEAR(T.inverse().getTransformationMatrix(), T_mat.inverse().eval(), kEpsilon));
  ...
  for (int i = 0; i < v_vectorized.cols(); ++i) {
    EXPECT_TRUE(EIGEN_MATRIX_NEAR(v_result.col(i), T * v_vectorized.col(i), kEpsilon));
  }
}

TEST(TestTransformation2D, TestCast) {
  const Transformation2D T_double(r, t);
  const Transformation2DTemplate<float> T_float = T_double.template cast<float>();

  EXPECT_NEAR(T_double.getRotation().angle(), static_cast<double>(T_float.getRotation().angle()),
              1e-5);
  EXPECT_TRUE(
      EIGEN_MATRIX_NEAR(T_double.getPosition(), T_float.getPosition().cast<double>(), 1e-5));
}
