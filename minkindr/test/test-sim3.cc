...
TEST(QuatSimTransformTest, Transform) {
  ...
  transformation_matrix << 1, 0, 0, 0, 0, 0, -1, 1, 0, 1, 0, 0, 0, 0, 0, 1;
  const QuatSimTransform sim3(QuatTransformation(transformation_matrix), 2.0);
...
TEST(QuatSimTransformTest, Inverse) {
  const RotationQuaternion q(Eigen::Quaterniond(0.64491714, 0.26382416, 0.51605132, 0.49816637));
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR(sim3 * (sim3.inverse() * input), input, 1e-6));
  EXPECT_TRUE(EIGEN_MATRIX_NEAR(sim3.inverse() * (sim3 * input), input, 1e-6));
}
